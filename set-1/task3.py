    ...
    with open(loan_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            loans.append({
                ...
                'renewed': row.get('renewed', 'False')  # Track if loan has been renewed
            })
    return loans
...
def get_user_fines(loans: list, books: dict, user_id: str) -> float:
    """
    Calculate total fines for a user (for display purposes).
    Includes fines for:
...
def get_user_unpaid_fines(loans: list, books: dict, user_id: str) -> float:
    """
    Calculate unpaid fines that block borrowing/renewal.
    Only includes fines for active loans that are currently overdue.
...
def display_menu(current_user: user.User):
    ...
    # Show Manage Library option only for Library staff
...
def search_by_keywords(keywords: list, books: dict) -> list:
    """
    Search for books by keywords.
    Returns list of book IDs sorted by:
    ...
    Returns:
        List of matching book IDs sorted by priority
    """
    if not keywords:
        return []
    ...
    matches = []
    for book_id, book_obj in books.items():
        # Get book keywords (split by colon)
        book_keywords = [kw.strip().lower() for kw in book_obj.keywords.split(':')]
        
        # Count matched keywords
        matched_count = sum(1 for kw in keywords_lower if kw in book_keywords)
    ...
    # Sort by priority
    matches.sort(key=lambda x: (-x['matched_count'], -x['year'], x['book_id']))
    
    return [m['book_id'] for m in matches]


def renew_loan(current_user: user.User, book_id: str, books: dict, loans: list):
    """
    Process renewing a loan.
    Extends due date by 5 days if eligible.
    
    Args:
        current_user: User renewing the book
        book_id: Book ID to renew
        books: Dictionary of book objects
    ...
    """
    # First, check if user has unpaid fines (user eligibility)
    fines = get_user_unpaid_fines(loans, books, current_user.user_id)
    if fines > 0:
        print("Renewal denied: You have unpaid fines.")
        return False
    
    # Then, check if loan record exists (loan validity)
    ...
    loan_to_renew = user_active_loans[0]
    
    # Check if book is overdue (loan eligibility)
    today = parse_date(user.TODAY)
    due_date = parse_date(loan_to_renew['due_date'])
    
    if today >= due_date:
        print("Renewal denied: This book is already overdue.")
        return False
    
    # Check if already renewed (loan eligibility)
    if loan_to_renew.get('renewed', 'False') == 'True':
    ...
    # Extend due date by 5 days
    new_due_date = due_date + datetime.timedelta(days=5)
    new_due_date_str = format_date(new_due_date)
    
    # Update loan record
    loan_to_renew['due_date'] = new_due_date_str
    loan_to_renew['renewed'] = 'True'
    
    print(f"Renew '{book_obj.title}' by {book_obj.author} ({book_obj.year}) successfully. New due date: {new_due_date_str}")
    return True
...
def borrow_and_return_console(current_user: user.User, books: dict, loans: list):
    """
    Interactive console for borrowing, returning, and renewing books.
    Commands: borrow <query>, return <book_id>, renew <book_id>, quit
    ...
    while True:
        ...
        if command == 'quit':
            ...
            # Attempt to renew
            renew_loan(current_user, book_id, books, loans)
...
def search_by_keywords_menu(books: dict):
    """
    Interactive menu for searching books by keywords.
    
    Args:
        books: Dictionary of book objects
    """
    keywords_input = input("Enter search keywords (separated by comma): ")
    
    # Parse keywords
    keywords = [kw.strip() for kw in keywords_input.split(',')]
    
    # Search for books
    matching_books = search_by_keywords(keywords, books)
    
    print(f"Found {len(matching_books)} book(s).")
    
    for idx, book_id in enumerate(matching_books, 1):
        book_obj = books[book_id]
        print(f"{idx}. {book_id} '{book_obj.title}' by {book_obj.author} ({book_obj.year}).")


def get_all_keywords(books: dict) -> set:
    """
    Extract all unique keywords from the book collection.
    ...
    for book_obj in books.values():
        keywords = [kw.strip().lower() for kw in book_obj.keywords.split(':')]
        all_keywords.update(keywords)
    return all_keywords


def detect_keywords_from_title(title: str, all_keywords: set) -> list:
    """
    Detect keywords in title by matching with existing keywords.
    
    Args:
        title: Book title
        all_keywords: Set of all existing keywords
    
    Returns:
        List of detected keywords sorted alphabetically
    """
    # Split title into words and normalize
    title_words = [word.strip().lower() for word in title.split()]
    
    # Find matching keywords
    detected = []
    for keyword in all_keywords:
        # Check if keyword appears as a word in the title
...
def generate_book_id(books: dict, book_type: str) -> str:
    """
    Generate a new book ID based on the highest existing ID.
    ...
    Returns:
        New book ID (e.g., 'P0020' or 'E0005')
    """
...
def add_book(books: dict, book_type: str):
    """
    Add a new book to the library.
    
    Args:
        books: Dictionary of book objects (will be modified)
        book_type: 'physical' or 'online'
    """
    ...
    # Detect keywords
    all_keywords = get_all_keywords(books)
    detected_keywords = detect_keywords_from_title(title, all_keywords)
    keywords_str = ':'.join(detected_keywords) if detected_keywords else ''
    
    if detected_keywords:
        print(f"Detected keywords: {keywords_str}")
    
    # Generate book ID
    book_id = generate_book_id(books, book_type)
    
    # Create book object
    new_book = book.Book(book_id, book_type, copies, title, authors, year, keywords_str)
    books[book_id] = new_book
    
    print(f"Adding {book_id} '{title}' by {authors} ({year}).")
...
def manage_library_console(users: dict, books: dict, loans: list):
    """
    Interactive console for library management.
    ...
    while True:
        ...
        if command == 'quit':
            ...
            book_type_input = parts[1]
            
            if book_type_input == 'physical':
                add_book(books, 'physical')
            elif book_type_input == 'online':
                add_book(books, 'online')
    ...
    while True:
        ...
        while True:
            ...
            if choice == '0':
                ...
                # Search by Keywords
                search_by_keywords_menu(books)
                display_menu(current_user)  # Re-display menu after action
            elif choice == '6':
                # Manage Library (only for Library staff)
                if isinstance(current_user, user.Staff) and current_user.is_library_staff():
                    manage_library_console(users, books, loans)
                    display_menu(current_user)  # Re-display menu after action
